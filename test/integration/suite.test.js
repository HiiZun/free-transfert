...
runner.test('Constructor validation - missing availability', async () => {
    try {
        new FreeTransfer({ path: './test/fixtures/test.dev' });
...
runner.test('Constructor validation - invalid availability', async () => {
    try {
        new FreeTransfer({ path: './test/fixtures/test.dev', availability: 50 });
...
runner.test('File upload from path', async () => {
    ...
    if (!fs.existsSync('./test/fixtures/test.dev')) {
        fs.writeFileSync('./test/fixtures/test.dev', 'test data for file upload');
    }
